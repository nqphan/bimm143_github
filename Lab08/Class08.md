...
the unsupervised learning technique covered in our last class.
...
There are 569 observations/pateinets in the dataset.
...
The plot shows some separation between two groups, but it’s a bit hard
to interpret since the axes PC1 and PC2 don’t represent specific
features. Color would make it easier to see the groups.

The `prcomp()` function to do PCA has a `scale-FALSE` default. In
general we nearly always want tot set this to TRUE so our analysis is
...
units of measurements are on different unit/scales.
...
The PC1 vs PC3 plot shows a similar pattern to the PC1 vs PC2 plot, but
the separation between groups is less clear. This suggests that PC3
explains less of the variation related to the groups.
...
K-means separates the two diagnoses fairly well but not perfectly. It
often produces similar groupings to hierarchical clustering, though
ward.D2 hierarchical clustering sometimes aligns better with the real
diagnosis labels.
...
![](Class08_files/figure-commonmark/unnamed-chunk-28-4.png)
...
For specificity PCA+ward.D2 had the highest For sensitivity PCA+ward.D2
also had the highets
...
From the dendrogram, the model has 4 clusters at a height of about 20
(the height where the horizontal cut creates 4 groups).
